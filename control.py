...
def robot_move(left_motor_PWR=0, right_motor_PWR=0, adjust_gain = 0.03):
    if ROBOT_MOVEMENT : 
        if SWAP_LR_MOTOR:
...
def robot_follow_direction(direction=None, normal_speed=0.3):
    left_PWR, right_PWR = calc_motor_PWR(direction, normal_speed)
    robot_move(left_PWR, right_PWR)
